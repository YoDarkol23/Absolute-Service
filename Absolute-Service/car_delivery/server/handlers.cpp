...
std::string handle_post_admin_documents(const std::string& body) {
    try {
        ...
        // Сохраняем в файл
        save_to_file("data/documents.json", documents);
...
std::string handle_delete_admin_documents(const std::string& body) {
    try {
        json request = json::parse(body);
        std::string category = request.value("category", "");
        ...
        for (auto it = documents.begin(); it != documents.end(); ++it) {
            if (it->value("category", "") == category && it->value("name", "") == name) {
                documents.erase(it);
                found = true;
                break;
            }
        ...
        if (!found) {
            return R"({"error": "Document with category )" + category + R"( and name )" + name + R"( not found"})";
        }
